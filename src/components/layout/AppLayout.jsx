...
export default function AppLayout({ children, currentView, onViewChange }) {
  ...
  const filteredNavItems = navItems.filter(item => {
    ...
    if (role === 'manager') return item.id !== 'users' && item.id !== 'config';
    if (role === 'cashier') return item.id === 'pos' || item.id === 'sales';
    if (role === 'seller') return item.id === 'pos' || item.id === 'validation';
    if (role === 'kitchen') return item.id === 'kds';
    return false;
  });

  const isFullScreenRole = role === 'cashier' || role === 'seller' || role === 'kitchen';
      ...
      {!isFullScreenRole && (
        <aside className="w-72 flex flex-col m-5 mr-0 rounded-[36px] shadow-neu bg-surface z-20 border border-white/5 overflow-hidden transition-all duration-500">
          ...
          <div className="p-6 mt-auto">
            <div 
              onClick={logout}
              className="shadow-neu-inset rounded-[28px] p-4 flex items-center gap-4 bg-surface/50 border border-white/5 group hover:bg-surface transition-colors cursor-pointer active-scale"
            >
              <div className={cn("h-12 w-12 rounded-[18px] flex items-center justify-center shadow-neu-glow-primary text-white font-black text-sm flex-shrink-0 border border-white/20", currentUser?.color || "bg-primary")}>
                {currentUser?.name?.charAt(0)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-black text-foreground truncate tracking-tight">{currentUser?.name}</p>
                <div className="flex items-center gap-1.5 opacity-40">
                  <div className="h-1.5 w-1.5 rounded-full bg-danger group-hover:animate-pulse" />
                  <p className="text-[9px] text-foreground-subtle font-black uppercase tracking-widest group-hover:text-danger transition-colors">Cerrar Sesión</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="h-24 flex items-center justify-between px-10 z-20 relative bg-surface/80 backdrop-blur-2xl m-5 mb-0 rounded-[32px] shadow-neu border border-white/5">
          ...
          <div className="flex items-center gap-6">
            ...
            {/* Render top navigation actions based on role */}
            {isFullScreenRole ? (
              <>
                {role === 'kitchen' ? (
                  // Kitchen: only logout
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 bg-surface shadow-neu-inset px-4 py-2 rounded-[16px] border border-white/5">
                      <div className="h-2 w-2 rounded-full bg-red-500 animate-pulse" />
                      <span className="text-[11px] font-black text-foreground-subtle uppercase tracking-widest">Cocina Activa</span>
                    </div>
                    <button 
                      onClick={logout}
                      className="px-6 py-3.5 rounded-[20px] text-[11px] font-black uppercase tracking-[0.15em] text-danger hover:text-white bg-surface shadow-neu border border-white/5 transition-all flex items-center gap-3 active-scale hover:bg-danger"
                    >
                  ...
                  <>
                    <button 
                      onClick={() => onViewChange(role === 'cashier' ? 'sales' : 'validation')}
                    ...
                    >
                      {role === 'cashier' ? <History className="h-4 w-4" /> : <ShieldCheck className="h-4 w-4" />}
                      {role === 'cashier' ? 'Auditoría' : 'Validar Ticket'}
                    </button>
                    <button 
                      onClick={logout}
                      className="px-6 py-3.5 rounded-[20px] text-[11px] font-black uppercase tracking-[0.15em] text-danger hover:text-white bg-surface shadow-neu border border-white/5 transition-all flex items-center gap-3 active-scale hover:bg-danger"
