...
import { Search, Plus, UserPlus, CreditCard, Clock, Key, Receipt, Mail, Phone, User } from "lucide-react";

export default function GuestsView() {
  const { guests, loading, addGuest, checkoutGuest, addChargeToTab, fetchGuests } = useGuestStore();
  ...
  const handleAddGuest = (e) => {
    e.preventDefault();
    if(!newGuestForm.name) return addToast('El nombre es requerido', 'error');
    addGuest(newGuestForm);
    setIsNewModalOpen(false);
    setNewGuestForm({ name: '', wristband: '', email: '', phone: '', adults: 1, children: 0 });
  ...
  const handleCheckout = () => {
    checkoutGuest(checkoutModalData.id, checkoutMethod);
    addSale({
       type: 'checkout',
       concept: `Checkout Cliente: ${checkoutModalData.name}`,
       amount: checkoutModalData.tabAmount,
       method: checkoutMethod
    });
    addToast(`Pago completado (${checkoutMethod}) y cuenta cerrada para ${checkoutModalData.name}`, 'success');
  ...
  return (
    <div className="h-full flex flex-col gap-10 bg-surface relative animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <div className="bg-surface shadow-neu rounded-[32px] p-8 border border-white/5 relative overflow-hidden group">
            ...
            <div className="text-4xl font-black text-success tracking-tighter">
               ${activeGuests.reduce((acc, g) => acc + (g.tabAmount || 0), 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
            </div>
         ...
         <div className="bg-surface shadow-neu rounded-[32px] p-8 border border-white/5 relative overflow-hidden group">
            ...
            <p className="text-[11px] font-black text-foreground-subtle uppercase tracking-[0.2em] mb-2 opacity-60">Checkout (Total)</p>
        ...
        <ScrollArea className="flex-1 scrollbar-none">
          {loading ? (
             <div className="flex flex-col items-center justify-center py-32 gap-4">
                ...
                <p className="text-xs font-black uppercase tracking-widest text-foreground-subtle">Cargando Clientes...</p>
             </div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface/50 backdrop-blur-md sticky top-0 z-10 border-b border-white/5">
                  <th className="px-10 py-8 text-[11px] font-black text-foreground-subtle uppercase tracking-[0.2em] opacity-40">Identidad / Contacto</th>
                ...
                {filteredGuests.map(guest => (
                  <tr key={guest.id} className="group hover:bg-white/[0.02] transition-all duration-300">
                    <td className="px-10 py-6">
                      <div className="flex items-center gap-5">
                         ...
                         <div>
                            ...
                            <p className="text-[9px] font-black text-foreground-subtle uppercase tracking-widest opacity-40 mt-1">
                              {guest.email || 'Sin Correo'} | {guest.phone || 'Sin Teléfono'}
                            </p>
                         </div>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-6 text-center font-black text-base text-foreground tracking-tighter">
                      ${(guest.tabAmount || 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-8 py-6 text-center">
                      {guest.status === 'active' ? (
                      ...
                      <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                         {guest.status === 'active' && (
                           <>
                             ...
                             <button 
                               onClick={() => {
                                 setCheckoutModalData({id: guest.id, name: guest.name, tabAmount: guest.tabAmount});
      ...
      {/* Nuevo Registro */}
      <Modal isOpen={isNewModalOpen} onClose={() => setIsNewModalOpen(false)} title="Registro de Cliente / Huésped">
        <form onSubmit={handleAddGuest} className="space-y-6 p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
               <label className="text-[10px] font-black text-foreground-subtle uppercase tracking-[0.2em] ml-2 mb-2 block opacity-60">Nombre Completo / Titular</label>
               <div className="relative">
                 ...
                 <input 
                   ...
                   className="w-full bg-surface shadow-neu-inset rounded-[20px] pl-12 pr-6 py-4 text-sm font-black outline-none border border-white/5 focus:ring-4 focus:ring-primary/5 transition-all"
                 />
               </div>
            </div>
            <div>
               <label className="text-[10px] font-black text-foreground-subtle uppercase tracking-[0.2em] ml-2 mb-2 block opacity-60">ID Digital / Pulsera</label>
               <div className="relative">
                 <Key className="absolute left-5 top-1/2 -translate-y-1/2 h-4 w-4 text-primary" />
                 <input 
            ...
            <div>
               ...
               <div className="relative">
                 <Phone className="absolute left-5 top-1/2 -translate-y-1/2 h-4 w-4 text-primary" />
                 <input 
                   type="tel"
            ...
            <div className="md:col-span-2">
               <label className="text-[10px] font-black text-foreground-subtle uppercase tracking-[0.2em] ml-2 mb-2 block opacity-60">Correo Electrónico</label>
               <div className="relative">
                 <Mail className="absolute left-5 top-1/2 -translate-y-1/2 h-4 w-4 text-primary" />
                 <input 
                   type="email"
            ...
            <div>
               <label className="text-[10px] font-black text-foreground-subtle uppercase tracking-[0.2em] ml-2 mb-2 block opacity-60">Adultos</label>
               <input 
                 type="number" min="1"
                 value={newGuestForm.adults}
                 onChange={e => setNewGuestForm({...newGuestForm, adults: parseInt(e.target.value) || 1 })}
                 className="w-full bg-surface shadow-neu-inset rounded-[20px] px-6 py-4 text-sm font-black outline-none border border-white/5 focus:ring-4 focus:ring-primary/5 transition-all"
               />
            </div>
            <div>
               <label className="text-[10px] font-black text-foreground-subtle uppercase tracking-[0.2em] ml-2 mb-2 block opacity-60">Menores</label>
               <input 
                 type="number" min="0"
          ...
          <button type="submit" className="w-full py-5 flex items-center justify-center gap-3 bg-primary text-primary-foreground font-black text-[12px] uppercase tracking-[0.2em] shadow-neu-glow-primary rounded-[22px] active-scale transition-all hover:opacity-95">
        ...
        {checkoutModalData && (
          <div className="space-y-10 p-4">
            <div className="bg-surface shadow-neu rounded-[32px] p-10 border border-white/5 relative overflow-hidden">
               ...
               <div className="space-y-6 relative z-10">
                  ...
                  <div className="flex flex-col gap-4 pt-4">
                    <p className="text-[10px] font-black text-foreground-subtle uppercase tracking-[0.2em] opacity-60 ml-2">Método de Liquidación</p>
                    <div className="grid grid-cols-2 gap-4">
                      <button 
                        onClick={() => setCheckoutMethod("cash")}
                        className={`py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest border transition-all ${checkoutMethod === 'cash' ? 'bg-primary text-primary-foreground shadow-neu-glow border-primary' : 'bg-surface text-foreground shadow-neu border-white/5'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest border transition-all ${checkoutMethod === 'card' ? 'bg-primary text-primary-foreground shadow-neu-glow border-primary' : 'bg-surface text-foreground shadow-neu border-white/5'}`}
                      >
                        <div className="flex flex-col items-center gap-2">
                          <CreditCard className="w-4 h-4" />
                          Tarjeta
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                 <Receipt className="w-5 h-5" /> Confirmar Pago y Cerrar Cuenta
              </button>
              <button 
